...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function create() {
      ...
      for (var i = 0; i < 35; i++) {
        days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="mark"></div><div class="mark2"></div><div class="day"></div><div class="lunar-day"></div></div>';
      }
      //添加下拉框数据
          ...
          daysElement.forEach(function (v, i) {
            ...
            if (data[year + '-' + currentmonth + '-' + day]) {
              // v.classList.add('sc-mark');
                v.querySelector('.mark').innerHTML = data[year + '-' + currentmonth + '-' + day]
	            if(data[year + '-' + currentmonth + '-' + day]!='未登录')
		            v.querySelector('.mark').classList.add("green")
	            else v.querySelector('.mark').classList.remove("green")
              v.title = data[year + '-' + currentmonth + '-' + day];
            } else {
	            v.querySelector('.mark').innerHTML =''
	            v.classList.remove('sc-mark');
              v.title = '';
            }
            if (data2[year + '-' + currentmonth + '-' + day]) {
                console.log('data2',data2[year + '-' + currentmonth + '-' + day])
                // v.classList.add('sc-mark');
                v.querySelector('.mark2').innerHTML = data2[year + '-' + currentmonth + '-' + day]

            } else {
                v.querySelector('.mark2').innerHTML =''
    ...
    //获取用户点击的日期
	  ...
	  key: 'addMarks',
	  value: function addMarks(obj, year, month) {
	  	for(var key in obj){
		    this._options.mark[key] = obj[key];
	    }
      ...
      value: function addMarks(obj, year, month) {
          for(var key in obj){
              this._options.mark2[key] = obj[key];
          }
          // this.update(year, month);
          this.update(year, month);
      }
