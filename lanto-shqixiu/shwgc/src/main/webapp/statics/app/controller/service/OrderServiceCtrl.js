Ext.define('app.controller.service.OrderServiceCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'service.OrderServiceMngPanel'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=service.OrderServiceMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=service.OrderServiceMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=service.OrderServiceMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=service.OrderServiceMngPanel]': {
				afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=service.OrderServiceMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
        	'panel[itemId=service.OrderServiceMngPanel] button[itemId=searchBtn]': {
            	click: this.doSearch 
            },
			'panel[itemId=service.OrderServiceMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=service.OrderServiceEdit] button[itemId=saveBtn]':{
				click: this.doSave
			}
		});
	},
	...
	doEdit:function(button){ 
		...
		var mds = ret.result;
		var win =	Ext.create('app.view.service.OrderServiceEdit',{title:'<i class="' + button.iconCls + '"></i> 服务确认'});
		win.show();
		win.loadData(mds);
	},
	...
	doSave : function(btn){
		...
		Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
			if(flag=='yes'){
				var url = 'service/orderservice/save';
	...
	doDelete : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				...
				var url = 'service/orderservice/delete';
