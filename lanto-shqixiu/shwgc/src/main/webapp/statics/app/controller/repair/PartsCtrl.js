Ext.define('app.controller.repair.PartsCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'repair.PartsMngPanel'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=repair.PartsMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=repair.PartsMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=repair.PartsMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=repair.PartsMngPanel]': {
				afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=repair.PartsMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
        	'panel[itemId=repair.PartsMngPanel] button[itemId=searchBtn]': {
            	click: this.doSearch 
            },
			'panel[itemId=repair.PartsMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=repair.PartsEdit] button[itemId=saveBtn]':{
				click: this.doSave
			}
		});
	},
	...
	doAdd : function(button){
		var win =	Ext.create('app.view.repair.PartsEdit',{title:'<i class="' + button.iconCls + '"></i> 配件信息编辑'});
		win.show();
	},
	//跳转修改页面
	...
	doDelete : function(button){
		...
		for(var i=0;i<mds.length;i++){
			var record = mds[i];
			if(i == 0){
				ids = record.data.PART_ID;
			}else{
				ids = ids + "," + record.data.PART_ID;
			}
		}
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				var store = grid.getStore();
				var url = 'repair/parts/delete';
