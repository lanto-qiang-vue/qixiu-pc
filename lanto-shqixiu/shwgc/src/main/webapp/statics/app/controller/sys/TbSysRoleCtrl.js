Ext.define('app.controller.sys.TbSysRoleCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'sys.TbSysRoleMngPanel',
		'sys.TbSysRoleEdit'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=sys.TbSysRoleMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=sys.TbSysRoleMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=sys.TbSysRoleMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=sys.TbSysRoleMngPanel]': {
                afterrender : app.utils.CommonUtil.setPanelFuncPower
            },
            'panel[itemId=sys.TbSysRoleEdit]': {
                afterrender : app.utils.CommonUtil.setPanelFuncPower
            },
			'panel[itemId=sys.TbSysRoleMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
			'panel[itemId=sys.TbSysRoleMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=sys.TbSysRoleMngPanel] panel[itemId=spanel] button[itemId=searchBtn]':{
				click: this.doSearch
			},
			'window[itemId=sys.TbSysRoleEdit] button[itemId=saveBtn]': {
				click: this.doSave
			},
			'window[itemId=sys.TbSysRoleEdit] treepanel':{
	...
	doAdd:function(button){ 
		var win =	Ext.create('app.view.sys.TbSysRoleEdit');
		win.show();
		win.loadData(null);
	},
	//跳转修改页面
	...
	doSave : function(btn){
		...
		Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
			if(flag=='yes'){
		    	...
		    	var url = "common/sys/RoleManager/modifyRole.json";
		    	var count = 0;
				var funcs = [];	
				...
				var url = 'client/sys/tbsysrole/save';
	...
	doDelete : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				...
				var url = 'client/sys/tbsysrole/delete';
