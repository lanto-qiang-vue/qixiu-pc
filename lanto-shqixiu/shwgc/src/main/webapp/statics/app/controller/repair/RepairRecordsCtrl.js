Ext.define('app.controller.repair.RepairRecordsCtrl',{
	...
	refs: [{
		...
		ref : 'workPics',
		selector : 'panel[itemId=repair.RepairWorkManage] dataview[itemId=repairphoto]'
	},{
		ref : 'gridPanel',
	...
	init: function(ctrl) {
		this.control({
			...
			'panel[itemId=repair.RepairWorkManage] button[itemId=uploadBtn]':{
				click: this.workPhoto
			},
			'panel[itemId=repair.WorkPhotoUpload] button[itemId=saveBtn]': {
				click: this.saveWorkPhoto
			},
			'panel[itemId=repair.RepairWorkManage] button[itemId=getPicBtn]':{
	...
	workPhoto : function(btn){
		var data=btn.up('window').down('form').getValues();
		var params = {RECORD_ID : data.RECORD_ID};
		var win = Ext.create('app.view.repair.WorkPhotoUpload');
		win.setPosition(0,0);
		win.loadData(params);
	},
	//上传抓拍照片保存按钮
