...
Ext.define('app.ux.UploadPanel',{
	...
	upload_url :'client/common/fileUpload/upload',
		...
		items : [{
					...
					handler : function(e) {
						...
						Ext.MessageBox.confirm('系统提示','确认清空列表吗？',function(flag){
							if(flag=='yes'){
								var url = "client/common/fileUpload/remove";
...
//							'所有文件大小之和不能超过' + app.utils.AttachCodes.file_total_size + '</font>'
	...
	store : Ext.create("Ext.data.Store", {
		...
		proxy : {
			type : "ajax",
			url : 'client/common/fileUpload/queryFile',
			actionMethods : {
				create : "POST",
				read : "POST",
				update : "POST",
        ...
        fields : ['id','fileUploadInfoId','name','type','size','percent','status','fileName','fileUrl','fileId','attrUser','tag']
    }),
				...
				items : [{
					...
					handler : function(b, c, a) {
						...
						if(!Ext.isEmpty(record.data.fileUploadInfoId)){
							...
							if(flag=='yes'){
								var url = "client/common/fileUpload/delete";
					...
					handler : function(b, c, a) {
						...
						if(params.status == -4) {
							app.utils.CommonUtil.exportStandardSubmit(form,'client/common/fileUpload/downloadFile',params);
						}else {
