Ext.define('app.view.corp.CorpJoinEdit',{
	extend:'Ext.window.Window',
	alias: 'widget.corp.CorpJoinEdit',
	itemId: 'corp.CorpJoinEdit',
	title: '入网申请资料',
	...
	loadAddData : function(){
		var me = this;
		me.down("#map_position").update('<iframe height="100%" width="100%" src="client/corp/corpjoin/position" scrolling="yes" frameborder="0" id="corpjoinbmapFrame" name="bmapFrame"></iframe>');
		app.utils.AjaxCallbackUtil.ajaxTitleCallback('client/corp/corpjoin/getCorpInfo',null,me,"加载中...",function(ret){
			if( ret.success ){
				if(ret.data){
					delete ret.data.JOIN_ID;
				}
				me.down('form').getForm().setValues(ret.data);
      		}
		});
	},
	loadData : function(record){
		...
		me.down("#map_position").update('<iframe height="100%" width="100%" src="client/corp/corpjoin/position?joinId=' + record.get('JOIN_ID') + '" scrolling="yes" frameborder="0" id="corpjoinbmapFrame" name="bmapFrame"></iframe>');
	...
	getImageBtn:function(){
		...
		return {
	        ...
	        pluploadConfig:{       	
				...
				url :  '../client/corp/corpjoin/uploadImages',
...
//								container: document.getElementById('newpart_area'), // ... or DOM Element itself
				...
				init: {
					...
					FileUploaded : function(up, file, responseObject) {
						...
						if(count == 0){
							me.down('#front_image').update('<img src="/attach/' + data.data + '" style="max-width:500px;max-height:450px;"/>');
