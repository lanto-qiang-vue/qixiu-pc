package com.lanto.shqixiu.shwgc.interceptor;
...
//		logger.info("拦截URL成功……");
		...
		if(url.matches(urREX) && url.indexOf("/client/common/fileUpload/upload") == -1){
...
//				logger.info("拦截URL发现cookie已失效-----将跳转到登录页面--");
	...
	public String getLoginUser(HttpServletRequest request) throws UnsupportedEncodingException{
		return CommonUtils.checkNull(getCookieValue(Constant.CLINET_LOGIN_USER_COOKIE_VALUE,request));
	}
	public String getLoginUserCode(HttpServletRequest request) throws UnsupportedEncodingException{
		return CommonUtils.checkNull(getCookieValue(Constant.CLINET_LOGIN_USERCODE_COOKIE_VALUE,request));
	}
	public String getLoginCorpId(HttpServletRequest request) throws UnsupportedEncodingException{
		return CommonUtils.checkNull(getCookieValue(Constant.CLINET_LOGIN_CORPID_COOKIE_VALUE,request));
	}
	public String getLoginCorpName(HttpServletRequest request) throws UnsupportedEncodingException{
		return CommonUtils.checkNull(getCookieValue(Constant.CLINET_LOGIN_CORPNAME_COOKIE_VALUE,request));
	}
