...
import com.lanto.shqixiu.shwgc.model.bean.CheckedNode;
import com.lanto.shqixiu.shwgc.model.bean.Dict;
import com.lanto.shqixiu.shwgc.model.bean.FuncNode;
...
import com.lanto.shqixiu.shwgc.service.sys.CommonService;
...
@Controller
@RequestMapping(value="client/sys/common",produces = "text/html;charset=UTF-8")
public class CommonController extends BaseCon {
	...
	public Object getMenus(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			Integer userId = super.getClientLoginInfo().getUserId();
	...
	public Object getLoginUser(HttpServletRequest request, HttpServletResponse response) throws Exception{
		try{
			ClientLoginInfo user = super.getClientLoginInfo();
			if(user.getUserId() == null){
				return super.getOutData(null);
			}
			Map userInfo = loginService.getLoginUserInfo(user.getUserId());
	...
	public Object areaTree(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try{
			...
			TbBdArea po = service.getAreaModel(super.getClientLoginInfo().getAreaCode());
