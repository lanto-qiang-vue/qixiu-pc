...
function getAjaxData(page, limit){
	...
	accessPost(baseu+ '/maintain/getMyOnsiteOrderlist', JSON.stringify(param), function (res) {
		console.log('res: ', res);
		...
		for(var i = 0; i < datas.length; i++){
			str += '<tr> ' +
				'<td style="text-align:center;">' + (datas[i].servicecontent || "") +'</td>' +
				...
				'<td style="text-align:center;">' + formatDate(datas[i].onServiceTime, "yyyy-MM-dd")  +'</td>' +
				// '<td style="text-align:center;"><font color="#FF5722">' + formatStatusToLabel(datas[i].status) +'</font></td>' +
				'<td style="text-align:center;"><font color="#FF5722">' + (datas[i].handleStatus) +'</font></td>' +
...
// 		limit:nums
// 	};
// 	var url = ctx + "/center/getMyOrders.do";
// 	ajaxEvent(url,params,function(ret){
// 		datas = ret.data;
// 		var str = '';
// 	    for(var i = 0; i < datas.length; i++){
// 	    	str += '<tr> ' +
// 		                '<td><div style="text-overflow: ellipsis;overflow: hidden;white-space: nowrap;width:300px;">' + datas[i].SERVICE_CONTENT +'</div></td>' +
// 		                '<td style="text-align:center;">' + datas[i].SERVICE_TIME_ +'</td>' +
// 		                '<td style="text-align:center;"><font color="#FF5722">' + datas[i].STATUS_ +'</font></td>' +
// 		                '<td>' +
// 		                	'<button class="layui-btn layui-btn-mini layui-btn-normal view" data-id="' + datas[i].SERVICE_ID + '"><i class="layui-icon">&#xe63c;</i> 查看 </button>' +
// 		                	 '<button class="layui-btn layui-btn-mini layui-btn-danger del" data-id="' + datas[i].SERVICE_ID + '"><i class="layui-icon">&#xe640;</i> 删除 </button>' +
// 		                '</td> ' +
// 		            '</tr>';
//
// 	    }
...
function formatStatusToLabel2(statusCode){
    var statusLabel = '';
    switch(statusCode){
        case 1:
            statusLabel = '新增';
