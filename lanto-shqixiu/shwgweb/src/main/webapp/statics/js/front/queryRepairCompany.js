...
var listDataFirstTime = function () {
    layer.load();
    var corpName = decodeURI(getUrlParam('searchText'));
        ...
        onSearchComplete : function () {
            ...
            if(searchResult){
                ...
                if(poi){
                    ...
                    $('#hidLat').val('31.249176');
                    $('#hidLng').val('121.487879');
                    // alert("无法定位");
                }
            } else{
                $('#hidLat').val('31.249176');
            ...
            layui.use('laypage', function(){
                ...
                listRangeCorpFirstTime(1,corpLimit,laypage);
            });
...
var listRangeCorpFirstTime = function (pageNo,pageSize,laypage,isRender) {
    layer.load();
    ...
    var corpName = getUrlParam('searchText')?decodeURI(getUrlParam('searchText')):'';
    var compSort = getUrlParam('compSort')?getUrlParam('compSort'):'';
    var compType = getUrlParam('compType')?getUrlParam('compType'):'';
    var compArea = getUrlParam('compArea')?getUrlParam('compArea'):'';
    var carBrand = getUrlParam('carBrand')?decodeURI(getUrlParam('carBrand')):'';
    var searchOption = getUrlParam('searchOption')?getUrlParam('searchOption'):'';

    // 搜索框内容
    $("#searchtext").val(corpName ? corpName : '');

    // 搜索框前的选择框
    if (searchOption) {
        var searchOptionName = "";
        $("#selSearchOption dl dd").each(function(){
            var value = $(this).attr("data-value");
            if (value == searchOption) {
                $(this).attr("class", "select-this");
                searchOptionName = $(this).text().trim()
            }
        })
        console.log("searchOptionName: ", searchOptionName);
        $("#selSearchOption").attr("data-value", searchOption); // 企业排序
        $("#searchOption").val(searchOptionName);
    }
    // 企业排序
    if (compSort) {
        var compSortName = "";
        $("#sortField dl dd").each(function(){
            var value = $(this).attr("data-value");
            if (value == compSort) {
                $(this).attr("class", "select-this");
                compSortName = $(this).find("span").text()
            }
        })
        $("#sortField").attr("data-value", compSort); // 企业排序
        $("#sortField").attr("title", compSortName); // 企业排序名称
        $("#compsort").val(compSortName);
    }

    // 企业类型
    if (compType) {
        var compTypeName = "";
        $("#companycategory dl dd").each(function(){
            var value = $(this).attr("data-value");
            if (value == compType) {
                $(this).attr("class", "select-this");
                compTypeName = $(this).find("span").text()
            }
        })
        $("#companycategory").attr("data-value", compType); // 企业类型
        $("#companycategory").attr("title", compTypeName); // 企业类型名称
        $("#compType").val(compTypeName)
    }
    ...
    if (compArea) {
        var compAreaName = "";
        $("#comparea dl dd").each(function(){
            var value = $(this).attr("data-value");
            if (value == compArea) {
                $(this).attr("class", "select-this");
                compAreaName = $(this).find("span").text()
            }
        })
        $("#comparea").attr("data-value", compArea); // 企业区域
        $("#comparea").attr("title", compAreaName); // 企业区域名称
        $("#area").val(compAreaName);
    }


    // 车辆品牌
    if (carBrand) {
        $("#carbrand").attr("data-value", carBrand)
        var carBrandName = carBrand;
        if (carBrand) {
            carBrandName = carBrand.trim().replace(",","|");
            $("#carbrand").attr("title", carBrandName)
        }
        $("#brand span").html(carBrandName);
        console.log("carBrandName",carBrandName);
        var brandArr = carBrand.split(",");
        $(".select-dl a").each(function () {
            var value = $(this).attr("data-value");
            for(i in brandArr) {
                if (value == brandArr[i]) {
                    $(this).attr("class", "select-this");
    ...
    systemCall(function (sysTok) {
        var param = {
            ...
            sortField: compSort,
            companycategory: compType,
            corpAreaEq: compArea,
            magorBrandsLk: carBrand,
            starLevel:'',
            type: '164',
            limit: pageSize,
...
// }
function showDialog(corpId,corpName) {
    goToPage('/center/repairInfo?role_id=1&comment=yes', false, [1,2,3,4,5,6,7])
}
