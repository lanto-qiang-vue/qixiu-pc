	...
	<div class="zhengwu_r">
		<div class="zhengwu_t">
			<span class="sp_zw">通知管理</span> <span class="sp_wz">您所在位置：<a
				href="${ctx}/center/userInfo">车主中心</a> > <span>通知管理</span></span>
		</div>
		<div>
		...
		<div class="biaoge" style="overflow: auto">
			<table id="table1" class="layui-table" lay-size="sm" style="width: 940px">
				<colgroup>
					<col ><col width="120"><col width="200"><col width="120"><col width="120">
				</colgroup>
				<thead>
				<tr>
					<th>通知标题</th>
...
//                html+='<tr notid="'+datas[i].notifyId+'"><td>'+datas[i].title+'</td><td>'+(content ? content.substring(0,21) + '...' : '无')+'</td><td>'+(datas[i].nickName? datas[i].nickName: datas[i].mobile)+'</td><td>'+new Date(datas[i].sendtime).toISOString().split(".")[0].split("T")+'</td><td>'+(datas[i].read?'<span style="color: green">已读</span>':'<span style="color: red">未读</span>')+'</td></tr>'
//            }
...
//	    console.log($(this).attr("notid"))
        ...
        function search(page, limit, laypage) {
            ...
            console.log("myNotes--param",param);
            //查询我收到的通知
