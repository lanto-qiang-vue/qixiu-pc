...
<div class="Government archives">
	...
	<div class="zhengwu_r">
		<div class="zhengwu_t">
			<span class="sp_zw">预约管理</span> <span class="sp_wz">您所在位置：<a
				href="${ctx}/center/companyHome">企业中心</a> > <span>预约管理</span></span>
		</div>
...
<div id="reason" style="display: none">
	<div class="layui-form-item" style="margin-top: 20px">
		...
		<div class="layui-input-block" style="width: 200px">
			<input type="text" name="reason" placeholder="请输入（必填）"  class="layui-input">
		</div>
	</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#table2 tbody").on('click','tr', function () {
			if(onsiteServiceId){
				...
				accessPost(baseu+ '/maintain/setRepairCompany4OnsiteOrder', JSON.stringify(param), function (res) {
	...
	function search(limit, page, laypage) {
		...
		accessPost(baseu+ '/maintain/getOnsiteOrderlist', JSON.stringify(param), function (res) {
			handleIfUserNeedToLoginIn(res, '${ctx}', '${memberUri}');
			console.log(res)
			...
			for (var i in data){
				...
				var setCompanyButtonHtml = (roleId==4 && !data[i].companyid ? '<button class="layui-btn layui-btn-xs" onclick="setCompany(' + data[i].id + ')"  style="font-size:10px">指定维修企业</button>' : '');
                ...
                var acceptButtonHtml = (roleId==2&& data[i].handleStatus=="待处理"?'<button class="layui-btn layui-btn-normal layui-btn-xs" onclick="accept(' + data[i].id + ')" style="font-size:10px">接受</button>' : '' );
                var refuseButtonHtml = (roleId==2&& data[i].handleStatus=="待处理"?'<button class="layui-btn layui-btn-danger layui-btn-xs" onclick="refuse(' + data[i].id + ')"  style="font-size:10px">拒绝</button>': '' );
                html+='<tr>' +
						'<td>'+(parseInt((param.pageNo - 1) * param.pageSize + parseInt(i) + 1))+'</td>' +
						'<td>'+
							deleteRowButtonHtml +
//							replayButtonHtml +
							setCompanyButtonHtml +
                            acceptButtonHtml +
                            refuseButtonHtml +
						'</td>' +
						'<td>'+data[i].ownername+'</td> ' +
//						'<td>'+formatStatusToLabel(data[i].status)+'</td>' +
						'<td>'+data[i].handleStatus+'</td>' +
						'<td>'+data[i].contactmobile+'</td> ' +
						'<td>'+data[i].servicecontent+'</td>' +
						'<td>'+new Date(data[i].onServiceTime + 28800000).toISOString().replace("T", " ").substring(0,16)+'</td>' +
...
//						'<td>'+(data[i].createtime ? new Date(data[i].createtime) : '')+'</td>' +
	...
	function refuse(id) {
        ...
        $('#reason a').attr("rowid", id)
    }
    $('#reason a').click(function () {
	    if(!$('#reason input').val()){
            layer.msg('请填写拒绝原因！', {time: 2000, icon:5})
		    return
	    }
        acceptOrRefuse($('#reason a').attr("rowid"), false, $('#reason input').val())
    })

    function acceptOrRefuse(id, status, reason) {
        var param={
            accept: status,
            accessToken: localStorage.getItem("ACCESSTOKEN"),
            onSiteOrderId: id,
        ...
        accessPatch(baseu+"/maintain/onsiteOrderHandle", JSON.stringify(param),function (res) {
	        console.log(res)
	        if(res.code=='000000'){
	...
	function updateStatusToReply(id){
		...
		layer.confirm('确定要设置该数据状态至已回复吗?', {icon: 3, title:'提示'}, function(index){
			...
			accessPost(baseu+ '/maintain/updateOnsiteOrderStatus2Reply', JSON.stringify(param), function (res) {
				handleIfUserNeedToLoginIn(res, '${ctx}', '${memberUri}');
				console.log(res);
				search(10, currPage, laypage);
			})
...
//		var ii = layer.load();
		...
		layer.confirm('确定要删除该行数据吗?', {icon: 3, title:'提示'}, function(index){
			...
			accessPost(baseu+ '/maintain/deleteOnsiteOrder', JSON.stringify(param), function (res) {
				handleIfUserNeedToLoginIn(res, '${ctx}', '${memberUri}');
				console.log(res);
				search(10, currPage, laypage);
			})
