...
import com.lanto.shqixiu.shcheck.model.bean.FuncCheckedNode;
import com.lanto.shqixiu.shcheck.model.bean.TcCheckFunc;
import com.lanto.shqixiu.shcheck.model.po.TjSysRole;
import com.lanto.shqixiu.shcheck.service.sys.TbSysRoleService;


@Controller
@RequestMapping(value="check/sys/tbsysrole",produces = "text/html;charset=UTF-8")
public class SysRoleController extends BaseCon{

	private Logger logger = Logger.getLogger(SysRoleController.class);// 日志
	...
	public Object save(HttpServletRequest request,HttpServletResponse response) {
		try{
			...
			TjSysRole po = json.getPojo(data, TjSysRole.class);
	...
	public Object getCheckedMenus(HttpServletRequest request,HttpServletResponse response) {
		try{
			...
			List<TcCheckFunc> newlist = new ArrayList<TcCheckFunc>();
			TcCheckFunc rootfun = new TcCheckFunc();
			rootfun.setFuncName("功能列表");
			rootfun.setFuncId(10l);
			rootfun.setParFuncId(0l);
			newlist.add(rootfun);
			newlist.addAll(list);
			FuncCheckedNode root = getTree(newlist,ROLE_ID);
			return super.getOutData(root);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
	...
	*/
	...
	private void createWhere(List<SqlUnit> sList,HttpServletRequest request){
		...
		String roleDesc = CommonUtils.checkNull(request.getParameter("ROLE_DESC"));
		...
		sList.add(new SqlUnit("CORP_ID", " and CORP_ID in (?,?) ", super.getCheckLoginInfo().getCorpId()));
		sList.add(new SqlUnit("CORP_ID", "","all_corp"));
		
		if (!CommonUtils.isNullString(roleName)){
			sList.add(new SqlUnit("roleName", " and ROLE_NAME like ? ","%" + roleName.trim() + "%"));
		}
		if (!CommonUtils.isNullString(roleDesc)){
			sList.add(new SqlUnit("roleDesc", " and ROLE_DESC like ? ","%" + roleDesc.trim() + "%"));
		}
