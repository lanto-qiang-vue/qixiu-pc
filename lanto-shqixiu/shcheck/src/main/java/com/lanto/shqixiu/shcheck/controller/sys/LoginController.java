...
public class LoginController extends BaseCon {
	...
	public Object doLogin(HttpServletRequest request, HttpServletResponse response) throws Exception{
		try{
			...
			String sess_rand = (String)request.getSession().getAttribute("check_randcode");
			...
			if (b){
				...
				setCookie(Constant.CHECK_LOGIN_USER_COOKIE_VALUE,userInfo.get("USER_ID").toString());
				setCookie(Constant.CHECK_LOGIN_USERCODE_COOKIE_VALUE,userCode);
				setCookie(Constant.CHECK_LOGIN_USERNAME_COOKIE_VALUE,userInfo.get("USER_NAME").toString());
				setCookie(Constant.CHECK_LOGIN_CORPID_COOKIE_VALUE,userInfo.get("CORP_ID").toString());
				setCookie(Constant.CHECK_LOGIN_CORPNAME_COOKIE_VALUE,userInfo.get("CORP_NAME").toString());
				setCookie(Constant.CHECK_LOGIN_AREA_COOKIE_VALUE,userInfo.get("CORP_AREA").toString());
...
//				UdpGetClientMacAddr mac = new UdpGetClientMacAddr(remoteAddr);
	...
	public Object randcode(HttpServletRequest request,
		...
		try {
			...
			request.getSession().setAttribute("check_randcode", sRand);
			// 使图像生效
			g.dispose();
			// 输出图像到页面
...
//				UdpGetClientMacAddr mac = new UdpGetClientMacAddr(remoteAddr);
	...
	public Object logout(HttpServletRequest request, HttpServletResponse response) throws Exception{
		try{
			super.removeCookie(Constant.CHECK_LOGIN_USER_COOKIE_VALUE,response);
			super.removeCookie(Constant.CHECK_LOGIN_CORPID_COOKIE_VALUE ,response);
			super.removeCookie(Constant.CHECK_LOGIN_CORPNAME_COOKIE_VALUE ,response);
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
