...
Ext.define('app.ux.UploadPanel',{
	...
	upload_url :'check/common/fileUpload/upload',
		...
		items : [{
					...
					handler : function(e) {
						...
						Ext.MessageBox.confirm('系统提示','确认清空列表吗？',function(flag){
							if(flag=='yes'){
								var url = "check/common/fileUpload/remove";
...
//							'所有文件大小之和不能超过' + app.utils.AttachCodes.file_total_size + '</font>'
	...
	store : Ext.create("Ext.data.Store", {
		...
		proxy : {
			type : "ajax",
			url : 'check/common/fileUpload/queryFile',
			actionMethods : {
				create : "POST",
				read : "POST",
				update : "POST",
				...
				items : [{
					...
					handler : function(b, c, a) {
						...
						if(!Ext.isEmpty(record.data.fileUploadInfoId)){
							...
							if(flag=='yes'){
								var url = "check/common/fileUpload/delete";
					...
					handler : function(b, c, a) {
						...
						if(params.status == -4) {
							app.utils.CommonUtil.exportStandardSubmit(form,'check/common/fileUpload/downloadFile',params);
						}else {
