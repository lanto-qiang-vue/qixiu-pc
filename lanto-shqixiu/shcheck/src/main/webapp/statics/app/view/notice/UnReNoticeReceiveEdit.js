Ext.define('app.view.notice.UnReNoticeReceiveEdit',{
	...
	title: '您所在企业有需要回执的通知，请回执后再进行其它操作！',
	...
	//	me.down('form').loadRecord(record);
		...
		app.utils.AjaxCallbackUtil.ajaxLoadingCallback("client/notice/tbnoticereceive/getContent",{NOTICE_ID:record.data.NOTICE_ID},me.down('panel[itemId=contentPanel]'),function(ret){
			if( ret.success ){
	...
	loadHomePageData : function(id){
		var me = this;
		me.down('panel[itemId=contentPanel]').setLoading("正在加载通知内容...");
		app.utils.AjaxCallbackUtil.ajaxLoadingCallback("client/sys/homepage/getContent",{NOTICE_ID:id},me.down('panel'),function(ret){
			if( ret.success ){
