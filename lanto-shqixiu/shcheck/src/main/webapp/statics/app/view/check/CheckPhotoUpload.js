Ext.define('app.view.check.CheckPhotoUpload',{
	extend:'Ext.window.Window',
	alias: 'widget.check.CheckPhotoUpload',
	itemId: 'check.CheckPhotoUpload',
	title: '检测图片上传',
	...
	initComponent : function() {
		...
		me.items = [ {
			...
			items:[{
				...
				model : 'app.model.repair.RepairPhotoModel',
				layout:'fit',
				btns:[],
				//defaults:{width:210},
				items: [
					{xtype:'container',
						...
						lableWidth:40,
						items:[{xtype:'hidden',name:'IMAGE_PATH',itemId:'vehphoto'},
							{xtype:'hidden',name:'CHECK_ID',itemId:'CHECK_ID'},
							{xtype:'field',name:'IMAGE_INFO',fieldLabel:'照片说明'},
							{xtype:'datetimefield',name:'TAKE_TIME',fieldLabel:'拍照时间',allowBlank:false},
							{xtype:'image',
							width:480,
							colspan:2,
							itemId:'vehImage',
	...
	loadData : function(params){
		var me = this;
		me.down('form').getForm().findField('CHECK_ID').setValue(params.CHECK_ID);
		me.down('form').getForm().findField('TAKE_TIME').setValue(new Date(),'Y-m-d H:i:s');
		me.down('form').getForm().findField('IMAGE_INFO').setValue('检测中照片');
	},
	getImageBtn:function(){
		...
		return {
	        ...
	        pluploadConfig:{       	
				...
				url :  '../check/checkphoto/uploadImage',
