...
var trNum=0;
Ext.define('app.view.check.CheckRecordsEdit',{
	extend:'Ext.window.Window',
	alias: 'widget.check.CheckRecordsEdit',
	itemId: 'check.CheckRecordsEdit',
	config:{
		info:'',			//信息提示窗口的文字信息
...
//	autoScroll:true,
	...
	baseInfoPanel : function(){
        ...
        items:[{
			...
			items: [
		    				 ...
		    				 rowSelType : 'SINGLE', //单选
			   				 callback:function( records ){
			   	    			//将取到的值赋给页面
			   				 	...
			   				 	params.TRANS_CORP_NAME = params.CORP_NAME;
				...
				{fname:'TRANS_CORP_NAME',readOnly:true},
				{xtype:'datetimefield',name:'CHECK_START_TIME',fieldLabel:'检测开始时间',allowBlank:true,readOnly:true},
				{xtype:'datetimefield',name:'CHECK_END_TIME',fieldLabel:'检测结束时间',allowBlank:true,readOnly:true},
	            {fname:'CHECK_TYPE'},
	            {fname:'VEHICLE_LEVEL'},
	            {fname:'CHECK_RESULT'},
	            {fname:'CHECK_PRESON'}
	...
	loadVideo : function(isPlay){
		...
		if(data.CHECK_END_TIME==''){
			...
			me.down('#startBtn').setDisabled(false);
			me.down('#endBtn').setDisabled(false);
	...
	StartPlayback : function(channel) {
		var me=this;
		var startTime=me.down('form').getValues().CHECK_START_TIME;
		var endTime=me.down('form').getValues().CHECK_END_TIME;
