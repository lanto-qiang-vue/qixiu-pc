...
//					var userId = getCookieValue("EXT_MANAGE_LOGIN_USERID_COOKIE");
//					var userPw = getCookieValue("EXT_MANAGE_LOGIN_USERPW_COOKIE");
			...
			items : [{
				...
				items : [{
					...
					html:'<div style="line-height:30px;">' +
							...
							'<div style="font-size:18px;font-family: Arial,宋体;text-align:right;padding-right:200px;">汽车电子健康档案系统.</span><span style="font-size:15px;">' + title + '</div>' +
						'</div>'
				}, {
					id : "warnLbl",
					text : "",
					...
					items : [{
								...
								listeners: {
							        click: {
							            element: 'el',
							            fn: function(a,b,c){
							            	Ext.getCmp('randimage').setSrc('manage/randcode?d=' + new Date().getTime());
							            }
							        }
							    },
								width : 78,
										  	     ...
										  	     success: function(response, opts) {
										  	        ...
										  	        if(ret.success){
										  		        if(ret.data == true) {
															...
															var date = new Date();
															document.cookie = "EXT_MANAGE_LOGIN_USERID_COOKIE=; expires=" + date.toGMTString();
													        document.cookie = "EXT_MANAGE_LOGIN_USERPW_COOKIE=; expires=" + date.toGMTString();
										  		        	if(loginTag == 'trans'){
										  		        		window.location = 'transmain.html';
										  		        	}else{
