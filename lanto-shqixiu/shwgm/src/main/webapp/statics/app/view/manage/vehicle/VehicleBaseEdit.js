			...
			items:[{
				...
				items:[{
					...
					items:[{
			    			...
			    			var win = Ext.create("app.view.common.TransCorpSelWin",{
			    				 ...
			    				 rowSelType : 'SINGLE', //单选
				   				 callback:function( records ){
				   	    			...
				   	    			params.TRANS_CORP_ID = params.CORP_ID;
				   	    			params.IS_SINGLE='10041002';
				   	    			params.OWNER_NAME=null;
				   	    			params.OWNER_PHONE=null;
					  ...
					  {xtype:'hidden',name:'TRANS_CORP_ID',itemId:'TRANS_CORP_ID'},{
							xtype:'textfield',
							flex:1,
							fieldLabel:'车牌号码',
							vtype:'License',
				...
				items:[{
					...
					items:[{
						...
						width:155,
						inputValue: '10041001',
						uncheckedValue:'10041002',
						listeners:{
							change :function(box,v){
								if(v){
									box.up('form').down('#OWNER_NAME').setReadOnly(false);
									box.up('form').down('#OWNER_PHONE').setReadOnly(false);
									box.up('form').down('#CORP_NAME').setValue(null);
									box.up('form').down('#TRANS_CORP_ID').setValue(null);
									
								}else{
									box.up('form').down('#OWNER_NAME').setReadOnly(true);
									box.up('form').down('#OWNER_PHONE').setReadOnly(true);
								}
							}
						}
					}]
...
part1:function(){
  ...
  part2: function(){
 	   ...
 	   return {
		   ...
		   rowSelType : 'MULTI',
 		   title: '车辆维修登记表',
		   url: 'manage/vehicle/vehiclebase/regList',
		   columns: [
     ...
     part3: function(){
 	   ...
 	   return {
		   ...
		   rowSelType : 'MULTI',
 		   title: '车辆主要总成部件更换登记表',
		   url: 'manage/vehicle/vehiclebase/partChangeList',
		   columns: [
	  			    {xtype:'rownumberer', name:'RowNumber', text:'序号', width:50}, 
		 ...
		 btns:[
			      
			      {
					 xtype : 'button',
	  				 ...
	  				 handler : function(btn) {
		                ...
		                var win = btn.up('window');
		                var store = win.down('grid[itemId=partPanel]').getStore();
		                var editing = win.down('grid[itemId=partPanel]').plugins[0];
		                store.insert(store.getCount(),record);
		                editing.startEditByPosition({
				            row: store.getCount()-1,
				            column: 2
			      ...
			      },'-',
			      {
	  				 ...
	  				 handler : function(btn) {
		                var win = btn.up('window');
		                var grid = win.down('grid[itemId=partPanel]');
   ...
   part4: function(){
 	   ...
 	   return {
		   ...
		   rowSelType : 'MULTI',
 		   title: '车辆变更登记表 ',
		   url: 'manage/vehicle/vehiclebase/changeList',
		   columns: [
	  			    {xtype:'rownumberer', name:'RowNumber', text:'序号', width:50}, 
 ...
 part5: function(){
 	   ...
 	   return {
		   ...
		   rowSelType : 'MULTI',
 		   title: '车辆使用登记 ',
		   url: 'manage/vehicle/vehiclebase/usesList',
		   columns: [
	  			    {xtype:'rownumberer', name:'RowNumber', text:'序号', width:50}, 
	...
	part6: function(){
 	   ...
 	   return {
		   ...
		   rowSelType : 'MULTI',
 		   title: '车辆交通事故登记 ',
		   url: 'manage/vehicle/vehiclebase/accidentList',
		   columns: [
	  			    {xtype:'rownumberer', name:'RowNumber', text:'序号', width:50}, 
	...
	part7: function(){
 	   ...
 	   return {
		   ...
		   rowSelType : 'MULTI',
 		   title: '车辆驾驶员登记',
		   url: 'manage/vehicle/vehiclebase/driverList',
		   columns: [
	  			    {xtype:'rownumberer', name:'RowNumber', text:'序号', width:50}, 
	...
	loadData : function(record){
		...
		//me.down('form').items['paramPanel'].loadRecord(params);
		if(!Ext.isEmpty(record.get('PHOTO'))){
			me.down('image').setSrc("/attach/" + record.get('PHOTO'));
		}
		
		//加载技术参数数据
		var url = 'manage/vehicle/vehiclebase/vehicleParam';
...
//								container: document.getElementById('newpart_area'), // ... or DOM Element itself
	...
	doDelete : function(btn,grid,pid,tableName){
		...
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				var store = grid.getStore();
				var url = 'manage/vehicle/vehiclebase/deleteVehicleItems';
