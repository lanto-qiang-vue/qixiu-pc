Ext.define('app.view.Viewport', {
	...
	layout: 'border',
	id:'manage_viewport',
	requires : taskutils.appInfos.requires,
	listeners:{
		afterrender:function(){
			Ext.get('appOverlay').hide();
			var url = 'manage/sys/common/getLoginUser';
			if(loginTag == 'trans'){
				url = 'trans/sys/common/getLoginUser';
			}
			// 将全局登陆用户变量放入全局变量中
			app.utils.AjaxCallbackUtil.asyncCall(url,{}, function (Obj) {
				Ext.get('login_user_text').setHTML('<span data-qtip="' + Obj.data.USER_NAME + '">' + Obj.data.USER_NAME + '</span>');
...
//			            '</li>',
			        	...
			        	'<li class="headerService menu1" code="WBM_CS"><i></i><span class="new-con-p">帮助</span>',
			        		'<ul class="container">',
			                    ...
			                    '<li class="menu2" onclick="location.href=\'http://www.xzinfo.com.cn/download/gld_manual.docx\'"><a class="new-con header-quickStart">系统帮助</a></li>',
			        			//'<li class="tel">020-xxx-xxxx<br/>星期一至星期五 <br/>8：30-17：00 <br/>（法定节假日除外）</li>',
			        		'</ul> ',
			        	'</li>',
			        '</ul>',
			        '</div>',
...
//            }
	...
	initComponent : function() {
    	...
    	var todoPanel = Ext.create('app.view.' + tag + '.HomePageMngPanel',{
        	itemId: tag + '.HomePageMngPanel',
	    ...
	    // 加载字典
		app.utils.DictUtil.loadDicts('manage/sys/common/dict', {},
