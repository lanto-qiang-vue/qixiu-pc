Ext.define('app.view.manage.privehicle.RepairRecordsView',{
	extend:'Ext.window.Window',
	alias: 'widget.manage.privehicle.RepairRecordsView',
	itemId: 'manage.privehicle.RepairRecordsView',
	title: '电子健康档案',
...
//	autoScroll:true,
	...
	loadData : function(record){
		...
		ist.proxy.extraParams.RECORD_ID_eq=recordId;
		ist.loadPage(1);
		//加载配件表格
		var pst=me.down('#partsGrid').getStore();
		pst.proxy.extraParams.RECORD_ID_eq=recordId;
		pst.loadPage(1);
		//加载维修图片
		var store = me.down('dataview[itemId=repairphoto]').getStore();
		store.proxy.extraParams.RECORD_ID=recordId;
		store.load();
	},
	//维修项目标签页
	getItems : function(){
		...
		return {
			...
			title : '维修项目',
			model : 'app.model.privehicle.RepairItemsListModel',
			url : 'manage/privehicle/repairitemslist/list',
			columns : [
	...
	getParts : function(){
		...
		return {
			...
			layout : {type:'fit',align:'stretch'},
			title : '配件信息',
			model : 'app.model.privehicle.PartsListModel',
			url : 'manage/privehicle/partslist/list',
			columns : [
				{text:'序号',xtype:'rownumberer'},
				...
				{fname:'PART_PHOTO',flex:1,align:'center',
                	...
                	listeners:{
                		click:function(a,b,c,d,e,f){
							...
							var win = Ext.create('app.view.manage.privehicle.PartPhotoUpload');
            ...
            items:[{
				...
				store : Ext.create('Ext.data.Store', {
					...
					proxy: {
				        ...
				        url: 'manage/privehicle/repairphoto/photolist'
				    }
				}),
				tpl: [
		                '<tpl for=".">',
