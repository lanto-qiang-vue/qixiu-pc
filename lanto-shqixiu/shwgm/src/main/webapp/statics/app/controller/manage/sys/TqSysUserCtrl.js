Ext.define('app.controller.manage.sys.TqSysUserCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'manage.sys.TqSysUserMngPanel',
		'manage.sys.TqSysUserEdit'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=manage.sys.TqSysUserMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=manage.sys.TqSysUserMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=manage.sys.TqSysUserMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=manage.sys.TqSysUserMngPanel]': {
				afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=manage.sys.TqSysUserEdit]': {
			//	afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=manage.sys.TqSysUserMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
			'panel[itemId=manage.sys.TqSysUserMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=manage.sys.TqSysUserMngPanel] panel[itemId=spanel] button[itemId=searchBtn]':{
				click: this.doSearch
			},
			'window[itemId=manage.sys.TqSysUserEdit] button[itemId=saveBtn]': {
				click: this.doSave
			},
			'window[itemId=manage.sys.TqSysUserSetStatus] button[itemId=saveBtn]': {
				click: this.doSave
			}
		});
	},
	...
	doAdd:function(button){ 
		var win =	Ext.create('app.view.manage.sys.TqSysUserEdit');
		win.show();
	},
	//跳转修改页面
	...
	setStatus:function(button){ 
		...
		var mds = ret.result;
		var win =	Ext.create('app.view.manage.sys.TqSysUserSetStatus');
		win.show();
		win.loadData(mds);
	},
	doSave : function(btn){
		...
		Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
			if(flag=='yes'){
				var url = 'manage/sys/tqsysuser/save';
	...
	doDelete : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				...
				var url = 'manage/sys/tqsysuser/delete';
	...
	resetPass : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认要重置密码吗？<br/> <font color="red">重置后，登录密码为：123456</font>',function(flag){
			if(flag=='yes'){
				var store = grid.getStore();
				var url = 'manage/sys/tqsysuser/resetPass';
				app.utils.AjaxCallbackUtil.ajaxCallbackAll(url,{ids:ids},function(ret){
					if( ret.success ){
	...
	doCreateUsers : function(button){
		...
		Ext.MessageBox.confirm('警告','确认要生成企业用户吗？<br/> <font color="red">此功能根据企业道路经营许可证号作为登录帐号生成企业用户（未生成过帐号的企业）<br/>生成后的企业用户初始密码为：123456<br/>完成此操作可能要花费较长时间，请谨慎操作！</font>',function(flag){
			if(flag=='yes'){
				var url = 'manage/sys/tqsysuser/createUsers';
