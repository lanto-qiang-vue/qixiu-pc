Ext.define('app.controller.trans.sys.TqSysUserCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'trans.sys.TqSysUserMngPanel',
		'trans.sys.TqSysUserEdit'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=trans.sys.TqSysUserMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=trans.sys.TqSysUserMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=trans.sys.TqSysUserMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=trans.sys.TqSysUserMngPanel]': {
				//afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=trans.sys.TqSysUserEdit]': {
				//afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=trans.sys.TqSysUserMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
			'panel[itemId=trans.sys.TqSysUserMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=trans.sys.TqSysUserMngPanel] panel[itemId=spanel] button[itemId=searchBtn]':{
				click: this.doSearch
			},
			'window[itemId=trans.sys.TqSysUserEdit] button[itemId=saveBtn]': {
				click: this.doSave
			}
		});
	},
	...
	doAdd:function(button){ 
		var win =	Ext.create('app.view.trans.sys.TqSysUserEdit');
		win.show();
	},
	//跳转修改页面
	...
	doSave : function(btn){
		...
		Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
			if(flag=='yes'){
				var url = 'trans/sys/sysuser/save';
	...
	doDelete : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				...
				var url = 'trans/sys/sysuser/delete';
	...
	resetPass : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认要重置密码吗？<br/> <font color="red">重置密码后，该帐号的登录密码为：123456</font>',function(flag){
			if(flag=='yes'){
				var store = grid.getStore();
				var url = 'trans/sys/sysuser/resetPass';
				app.utils.AjaxCallbackUtil.ajaxCallbackAll(url,{ids:ids},function(ret){
					if( ret.success ){
