Ext.define('app.controller.manage.employee.TbEmployeeExpertCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'manage.employee.TbEmployeeExpertMngPanel',
		'manage.employee.TbEmployeeExpertEdit'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=manage.employee.TbEmployeeExpertMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=manage.employee.TbEmployeeExpertMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=manage.employee.TbEmployeeExpertMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=manage.employee.TbEmployeeExpertMngPanel]': {
				afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=manage.employee.TbEmployeeExpertEdit]': {
				//afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=manage.employee.TbEmployeeExpertMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
			'panel[itemId=manage.employee.TbEmployeeExpertMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=manage.employee.TbEmployeeExpertMngPanel] panel[itemId=spanel] button[itemId=searchBtn]':{
				click: this.doSearch
			},
			'window[itemId=manage.employee.TbEmployeeExpertEdit] button[itemId=saveBtn]': {
				click: this.doSave
			}
		});
	},
	...
	doAdd:function(button){ 
		var win =	Ext.create('app.view.manage.employee.TbEmployeeExpertEdit');
		win.show();
	},
	//跳转修改页面
	...
	doSave : function(btn){
		...
		Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
			if(flag=='yes'){
				var url = 'manage/employee/tbemployeeexpert/save';
	...
	doDelete : function(button){
		...
		for(var i=0;i<mds.length;i++){
			var record = mds[i];
			if(i == 0){
				ids = record.data.EXPERT_ID;
			}else{
				ids = ids + "," + record.data.EXPERT_ID;
			}
		}
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				var store = grid.getStore();
				var url = 'manage/employee/tbemployeeexpert/delete';
	...
	doExport : function(button){
		...
		var url = "manage/employee/tbemployeeexpert/doExport";
