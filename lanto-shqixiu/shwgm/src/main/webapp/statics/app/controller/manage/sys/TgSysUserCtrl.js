Ext.define('app.controller.manage.sys.TgSysUserCtrl',{
	extend:'Ext.app.Controller',
	config:{userId:null},		//用户ID
	views: [
		'manage.sys.TgSysUserMngPanel',
		'manage.sys.TgSysUserEdit'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=manage.sys.TgSysUserMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=manage.sys.TgSysUserMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=manage.sys.TgSysUserMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=manage.sys.TgSysUserMngPanel]': {
				afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=manage.sys.TgSysUserEdit]': {
				//afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=manage.sys.TgSysUserMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
			'panel[itemId=manage.sys.TgSysUserMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=manage.sys.TgSysUserMngPanel] panel[itemId=spanel] button[itemId=searchBtn]':{
				click: this.doSearch
			},
			'window[itemId=manage.sys.TgSysUserEdit] button[itemId=saveBtn]': {
				click: this.doSave
			},
			'window[itemId=manage.sys.TgSysUserSetStatus] button[itemId=saveBtn]': {
				click: this.doSave
			},
			'panel[itemId=manage.sys.TgSysUserVehicleList] grid[itemId=gpanel]':{
	...
	doAdd:function(button){ 
		var win =	Ext.create('app.view.manage.sys.TgSysUserEdit');
		win.show();
	},
	//跳转修改页面
	...
	doSave : function(btn){
		...
		Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
			if(flag=='yes'){
				var url = 'manage/sys/tgsysuser/save';
	...
	doDelete : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认删除吗？',function(flag){
			if(flag=='yes'){
				...
				var url = 'manage/sys/tgsysuser/delete';
	...
	resetPass : function(button){
		...
		Ext.MessageBox.confirm('系统提示','确认要重置密码吗？<br/> <font color="red">重置后，登录密码为：123456</font>',function(flag){
			if(flag=='yes'){
				var store = grid.getStore();
				var url = 'manage/sys/tgsysuser/resetPass';
				app.utils.AjaxCallbackUtil.ajaxCallbackAll(url,{ids:ids},function(ret){
					if( ret.success ){
	...
	setStatus:function(button){ 
		...
		var mds = ret.result;
		var win =	Ext.create('app.view.manage.sys.TgSysUserSetStatus');
		win.show();
		win.loadData(mds);
	},
	//工具栏已绑定车辆按钮
	vehicleList:function(button){ 
		var me = this;
		var grid = me.getGridPanel();
		var ret = app.utils.FunUtils.singleChecks(grid);
		if(!ret.success){
		...
		var mds = ret.result;
		me.config.userId=mds.get('USER_ID');	//保存所选记录的USER_ID
		var win =	Ext.create('app.view.manage.sys.TgSysUserVehicleList');
		win.show();
		win.loadData(mds);
	}
});
