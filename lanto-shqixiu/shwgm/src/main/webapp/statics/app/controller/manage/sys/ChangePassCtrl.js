Ext.define('app.controller.manage.sys.ChangePassCtrl', {
	extend: 'Ext.app.Controller',
	views: ['manage.sys.ChangePassMngPanel'],
	init: function(dms) {
		this.control({
			'panel[itemId=manage.sys.ChangePassMngPanel]': {
	           // afterrender : app.utils.CommonUtil.setPanelFuncPower
	        },
			'panel[itemId=manage.sys.ChangePassMngPanel] button[itemId=saveBtn]': {
	            click : this.doSave
	        }
		});
	},
	doSave : function(btn){
		try{
			...
			Ext.MessageBox.confirm('系统提示','确认要修改吗？',function(flag){
    			if(flag=='yes'){
					app.utils.AjaxCallbackUtil.ajaxCallbackAll('manage/sys/tmsysuser/updatePassword',params,function(ret){
						if( ret.data ){
