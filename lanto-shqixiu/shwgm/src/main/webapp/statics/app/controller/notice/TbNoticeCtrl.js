Ext.define('app.controller.notice.TbNoticeCtrl',{
	extend:'Ext.app.Controller',
	views: [
		'notice.TbNoticeMngPanel',
		'notice.TbNoticeEdit'],
	refs: [{
		ref : 'mngPanel',
		selector : 'panel[itemId=notice.TbNoticeMngPanel]'
	},{
		ref : 'searchPanel',
		selector : 'panel[itemId=notice.TbNoticeMngPanel] panel[itemId=spanel]'
	},{
		ref : 'gridPanel',
		selector : 'panel[itemId=notice.TbNoticeMngPanel] panel[itemId=gpanel]'
	}],
	init: function(ctrl) {
		this.control({
			'panel[itemId=notice.TbNoticeMngPanel]': {
				afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=notice.TbNoticeEdit]': {
				//afterrender : app.utils.CommonUtil.setPanelFuncPower
			},
			'panel[itemId=notice.TbNoticeMngPanel] grid[itemId=gpanel]': {
				beforerender : this.initGridSearchCondition
			},
			'panel[itemId=notice.TbNoticeMngPanel] grid[itemId=gpanel] toolbar[dock=top] button':{
				click: this.gridBtnsHandler
			},
			'panel[itemId=notice.TbNoticeMngPanel] panel[itemId=spanel] button[itemId=searchBtn]':{
				click: this.doSearch
			},
			'window[itemId=notice.TbNoticeEdit] button[itemId=saveBtn]': {
				click: this.doSave
			},
			'window[itemId=notice.TbNoticeEdit] button[itemId=sendBtn]': {
				click: this.doSend
			}
		});
	},
	...
	gridBtnsHandler : function(button, event, eOpts){
		...
		if( 'addBtn'==button.itemId ){
			...
			me.doDelete(button);
		}else if( 'noticeReceiveBtn' == button.itemId){
			me.receiveInfo(button);
		}else{
	...
	doAdd:function(button){ 
		var win =	Ext.create('app.view.notice.TbNoticeEdit');
		win.show();
	},
	//跳转修改页面
	...
	receiveInfo : function(btn){
		...
		var mds = ret.result;
		var win =	Ext.create('app.view.notice.NoticeReceiveInfo',{title:mds.data.NOTICE_TITLE});
		win.show();
		win.loadData(mds);
	},
	doSave : function(btn){
