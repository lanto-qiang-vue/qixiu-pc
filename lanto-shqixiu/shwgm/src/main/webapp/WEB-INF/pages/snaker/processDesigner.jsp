	...
	<head>
		...
		<script type="text/javascript">
			...
			function saveModel(data) {
				parent.window.Ext.MessageBox.confirm('系统提示','确认保存吗？',function(flag){
					if(flag=='yes'){
						$.ajax({
							type:'POST',
							url:"${ctx}/manage/sys/flow/deployXml",
							data:"model=" + data + "&id=${processId}",
							async: false,
							globle:false,
							error: function(){
								parent.window.Ext.Msg.alert('错误','数据处理错误！');
								return false;
							},
							success: function(data){
								if(data == 'true') {
									parent.window.Ext.ComponentQuery.query('panel[itemId=sys.FlowMngPanel] grid[itemId=gpanel]')[0].getStore().reload();
									parent.window.Ext.ComponentQuery.query('window[itemId=sys.FlowEdit]')[0].close();
								} else {
									parent.window.Ext.Msg.alert('错误','数据处理错误！');
								}
							}
...
<body>
<div id="toolbox" style="left:810px;top:400px;">
