...
import com.lanto.shqixiu.shwgm.service.manage.report.CommentReportService;
import com.lanto.shqixiu.shwgm.service.manage.report.CorpNumReportService;


@Controller
@RequestMapping(value="manage/report/commentreport",produces = "text/html;charset=UTF-8")
public class CommentReportController extends BaseCon{

	private Logger logger = Logger.getLogger(CommentReportController.class);// 日志
	...
	@ResponseBody
	public Object scoreList(HttpServletRequest request,HttpServletResponse response) {
		try{
			ManageLoginInfo info=super.getManageLoginInfo();
			PageUnit pUnit = super.PageInit();
			List<SqlUnit> sList = new ArrayList<SqlUnit>();
			createWhere(sList,request);
			List list = service.getScoreList(sList, pUnit);
			return super.getOutPage(list,pUnit);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
