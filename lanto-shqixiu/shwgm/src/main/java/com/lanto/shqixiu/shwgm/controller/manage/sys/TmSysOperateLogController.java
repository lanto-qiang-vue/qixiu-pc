...
@Controller
@RequestMapping(value="manage/sys/tmsysoperatelog",produces = "text/html;charset=UTF-8")
public class TmSysOperateLogController extends BaseCon{

	private Logger logger = Logger.getLogger(TmSysOperateLogController.class);// 日志

	@Resource
	private TmSysOperateLogService service;
	...
	public Object save(HttpServletRequest request,HttpServletResponse response) {
		try{
			...
			TmSysOperateLog po = json.getPojo(data, TmSysOperateLog.class);
	...
	private void createWhere(List<SqlUnit> sList,HttpServletRequest request){
		...
		if (!CommonUtils.isNullString(type)){
			sList.add(new SqlUnit("logId", " and OPERATOR_ID = ? ",super.getManageLoginInfo().getUserCode()));
		}
