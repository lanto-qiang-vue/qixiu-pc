...
import com.lanto.shqixiu.shwgm.service.trans.sys.TtSysRoleService;


@Controller
@RequestMapping(value="trans/sys/sysrole",produces = "text/html;charset=UTF-8")
public class TtSysRoleController extends BaseCon{

	private Logger logger = Logger.getLogger(TtSysRoleController.class);// 日志

	@Resource
	private TtSysRoleService service;
	...
	public Object save(HttpServletRequest request,HttpServletResponse response) {
		try{
			...
			TtSysRole po = json.getPojo(data, TtSysRole.class);
	...
	private FuncCheckedNode getRoot(List<TcCheckFunc> list,boolean flag) {
		for(TcCheckFunc node : list){
			if("10".equals(node.getFuncId().toString())){
	...
	private void createWhere(List<SqlUnit> sList,HttpServletRequest request){
		...
		sList.add(new SqlUnit("CORP_ID", " and CORP_ID in (?,?) ", super.getTransLoginInfo().getCorpId()));
