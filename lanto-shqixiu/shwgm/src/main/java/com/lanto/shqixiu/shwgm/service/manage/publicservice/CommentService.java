...
import com.lanto.shqixiu.shwgm.model.po.TbComment;
...
public class CommentService {
	...
	public List getList(List<SqlUnit> uSql, PageUnit page) throws SQLException{
		page.setOrderByColumn("INSERT_DATE");
		page.setOrderByType("DESC");
		return swdb.getPageList(uSql, page, "TB_COMMENT","COMMENT_ID");
	}
	...
	public void delete(String ids,ManageLoginInfo info){
		...
		for(String chid : chids){
			...
			pojo.setCommentId(Integer.valueOf(chid));
			pojo = swdb.getModelByPo(pojo);
			String logcontent = "【%s】删除了一条%s【%s】  ";
			logcontent = String.format(logcontent, info.getUserName(),"质量投诉信息",pojo.getContent());
			logService.addOperatorLog(info,"质量投诉信息", logcontent);
			TbComment po = new TbComment();
			po.setCommentId(Integer.valueOf(chid));
			swdb.deleteByPo(po);
		}
	}
}
