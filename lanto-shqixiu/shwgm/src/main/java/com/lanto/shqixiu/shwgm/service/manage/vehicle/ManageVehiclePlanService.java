...
@Service
public class ManageVehiclePlanService {
	...
	public List getList(List<SqlUnit> uSql, PageUnit page,String areaCode) throws SQLException{
		...
		String table = "(select a.*,q.CORP_NAME,b.TRANS_CORP_ID,b.PLATE_NUM,b.PLATE_COLOR,b.VEHICLE_TYPE," +
				"c.NAME VENDER_,d.NAME BRAND_,e.NAME XM_ from tv_vehicle_plan a left join tv_vehicle_base b on a.VEHICLE_ID=b.VEHICLE_ID " +
				"left join tb_trans_corp_info q on b.trans_corp_id=q.CORP_ID \n" +
				"left join db_repair_standard c on b.VENDER=c.ID \n" +
				"left join db_repair_standard d on b.BRAND=d.ID \n" +
				"left join db_repair_standard e on b.XM=e.ID where q.corp_area like '%"+areaCode+"%' \n) t";
		return service.getPageList(uSql, page, table,"PLAN_ID");
	}
	
	public List toPlanList(String plateNum) throws SQLException{
	...
	public List doExport(List<SqlUnit> uSql){
		return service.getList("select * from tv_vehicle_plan where 1=1", uSql);
	}
	...
	public TvVehiclePlan getModel(TvVehiclePlan po){
