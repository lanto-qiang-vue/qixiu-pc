...
import com.lanto.shqixiu.shwgm.service.corp.YsCorpInfoService;
import com.lanto.shqixiu.shwgm.util.excel.ExcelWriterPoiWriter;


@Controller
@RequestMapping(value="trans/transcorp/tccorpinfo",produces = "text/html;charset=UTF-8")
public class YsCorpInfoController extends BaseCon{

	private Logger logger = Logger.getLogger(TransCorpInfoController.class);// 日志

	@Resource
	private YsCorpInfoService service;
	...
	public Object save(HttpServletRequest request,HttpServletResponse response) {
		try{
			...
			//TbTransCorpInfo po = json.getPojoList(data, TbTransCorpInfo.class);
	...
	public Object doPrint(HttpServletRequest request,HttpServletResponse response) {
		try{	
			...
			TbCorpInfo corp = json.getPojo(data, TbCorpInfo.class);
	...
	private void createWhere(List<SqlUnit> sList,HttpServletRequest request){
		...
		if (!CommonUtils.isNullString(plateNum)){
			sList.add(new SqlUnit("plateNum", " and PLATE_NUM like ? ","%" + plateNum + "%"));
		}			
