...
import com.lanto.shqixiu.shwgm.service.manage.sys.TqSysUserService;
import com.lanto.shqixiu.shwgm.util.MD5;


@Controller
@RequestMapping(value="manage/sys/tqsysuser",produces = "text/html;charset=UTF-8")
public class TqSysUserController extends BaseCon{

	private Logger logger = Logger.getLogger(TqSysUserController.class);// 日志

	@Resource
	private TqSysUserService service;
	...
	public Object save(HttpServletRequest request,HttpServletResponse response) {
		try{
			...
			TqSysUser po = json.getPojo(data, TqSysUser.class);
			if(po.getUserId() != null ){
				...
				TbCorpInfo corp = service.getCorpById(po.getCorpId());
				po.setBlongArea(corp.getCorpArea());
				po.setRoleId(1);
				service.save(po);
			}
	...
	@ResponseBody
	public Object createUsers(HttpServletRequest request,HttpServletResponse response) {
		try{
			service.updateCreateUsers();
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
