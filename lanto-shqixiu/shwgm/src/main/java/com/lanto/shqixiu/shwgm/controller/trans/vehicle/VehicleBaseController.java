...
import com.lanto.shqixiu.shwgm.service.trans.vehicle.VehicleBaseService;
import com.lanto.shqixiu.shwgm.util.Constant;
import com.lanto.shqixiu.shwgm.util.URLConnectionUtil;
import com.lanto.shqixiu.shwgm.util.excel.ExcelWriterPoiWriter;


@Controller
@RequestMapping(value="trans/vehicle/vehiclebase",produces = "text/html;charset=UTF-8")
public class VehicleBaseController extends BaseCon{

	private Logger logger = Logger.getLogger(VehicleBaseController.class);// 日志

	@Resource
	private VehicleBaseService service;
	@Resource
			...
			if(CommonUtils.checkIsNotNullAndZero(po.getVehicleId())){
				...
				if(corp.getCreateQuatoCount()==null || "".equals(corp.getCreateQuatoCount())){
					...
					if(!quatoStr.split(",")[1].equals(String.valueOf(corp.getCorpId()))){
						...
						if(corpService.createQuatoMinus1(String.valueOf(corp.getCorpId()),po.getPlateNum(),super.getManageLoginInfo())){
							po.setUpdateDate(new Date());
							po.setTransCorpId(info.getCorpId());
							service.save(po);
						}
	...
	public Object save_change(HttpServletRequest request,HttpServletResponse response) {
		try{	
			...
			TransLoginInfo info = super.getTransLoginInfo();
			service.saveItem(parray,Integer.parseInt(vehicleId),TvVehicleChange.class,"changeId");
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
	...
	public Object save_partChange(HttpServletRequest request,HttpServletResponse response) {
		try{	
			...
			String parray = CommonUtils.checkNull(request.getParameter("parray"));
			TransLoginInfo info = super.getTransLoginInfo();
			service.saveItem(parray,Integer.parseInt(vehicleId),TvVehiclePartChange.class,"partId");
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
	...
	public Object save_driver(HttpServletRequest request,HttpServletResponse response) {
		try{	
			...
			String parray = CommonUtils.checkNull(request.getParameter("parray"));
			TransLoginInfo info = super.getTransLoginInfo();
			service.saveItem(parray,Integer.parseInt(vehicleId),TvVehicleDriver.class,"driverId");
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
	...
	public Object save_reg(HttpServletRequest request,HttpServletResponse response) {
		try{	
			...
			String parray = CommonUtils.checkNull(request.getParameter("parray"));
			TransLoginInfo info = super.getTransLoginInfo();
			service.saveItem(parray,Integer.parseInt(vehicleId),TvVehicleReg.class,"regId");
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
			return super.getOutException(e,"车辆维修登记保存出错");
		}
	}	
	...
	public Object save_accident(HttpServletRequest request,HttpServletResponse response) {
		try{	
			...
			String parray = CommonUtils.checkNull(request.getParameter("parray"));
			TransLoginInfo info = super.getTransLoginInfo();
			service.saveItem(parray,Integer.parseInt(vehicleId),TvVehicleAccident.class,"accidentId");
			return super.getOutData(true);
		}catch(Exception e){
			e.printStackTrace();
			logger.error(e);
			return super.getOutException(e,"车辆事故登记保存出错");
		}
	}	
