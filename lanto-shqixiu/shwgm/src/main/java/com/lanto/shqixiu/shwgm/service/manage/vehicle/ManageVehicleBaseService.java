...
import com.lanto.shqixiu.shwgm.model.bean.TrRepairGeneManageBean;
import com.lanto.shqixiu.shwgm.model.bean.TrRepairGenePartBeanPrint;
...
import com.lanto.shqixiu.shwgm.model.po.TvVehicleAccident;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleBase;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleChange;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleCheck;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleDriver;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleParam;
import com.lanto.shqixiu.shwgm.model.po.TvVehiclePartChange;
import com.lanto.shqixiu.shwgm.model.po.TvVehiclePlan;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleReg;
import com.lanto.shqixiu.shwgm.model.po.TvVehicleUses;
...
@Service
public class ManageVehicleBaseService {
	...
	public List getList(List<SqlUnit> uSql, PageUnit page,String areaCode) throws SQLException{
		...
		sql.append("(select *,'10171001' WARN_TYPE from v_vehicle_plan where MONTH_DIFF <1 and (corp_area like '%"+areaCode+"%' or corp_area is null)\n");
		sql.append("       UNION\n");
		sql.append("select *,'10171002' WARN_TYPE from v_vehicle_plan where MONTH_DIFF =1 and (corp_area like '%"+areaCode+"%' or corp_area is null)\n");
		sql.append("       UNION\n");
		sql.append("select *,'10171003' WARN_TYPE from v_vehicle_plan where REPAIR_CYCLE is null and (corp_area like '%"+areaCode+"%' or corp_area is null)\n");
		sql.append("       UNION\n");
		sql.append("select *,'10171004' WARN_TYPE from v_vehicle_plan where REPAIR_CYCLE is not null and LAST_REPAIR_DATE is null and (corp_area like '%"+areaCode+"%' or corp_area is null)\n");
		sql.append("       UNION\n");
		sql.append("select *,'10171005' WARN_TYPE from v_vehicle_plan where MONTH_DIFF >1 and (corp_area like '%"+areaCode+"%' or corp_area is null)) t\n");
	...
	public List getCheckList(String vehId){
		String sql = "SELECT * FROM tb_check_records WHERE VEHICLE_ID=? AND STATUS='10191002' order by CHECK_END_TIME desc,CHECK_ID desc";
		return service.findAll(sql, vehId);
	}
	
	public List getCheckList(String vehId,String year){
		String sql = "SELECT * FROM tb_check_records WHERE VEHICLE_ID=? AND STATUS='10191002' AND CHECK_START_TIME LIKE '%"+year+"%' order by CHECK_END_TIME desc,CHECK_ID desc";
		return service.findAll(sql, vehId);
	}
	...
	public List getPic(String corpId,String billCode,Integer channel_code){
		...
		em.setCorpId(Integer.parseInt(corpId));
