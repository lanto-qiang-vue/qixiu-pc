...
@Controller
@RequestMapping(value="manage/publicservice/complaint",produces = "text/html;charset=UTF-8")
public class ComplaintController extends BaseCon{

	private Logger logger = Logger.getLogger(ComplaintController.class);// 日志
	...
	@ResponseBody
	public Object getReportData(HttpServletRequest request,HttpServletResponse response) {
		try{
			String startDate = CommonUtils.checkNull(request.getParameter("ADD_DATE_gte"));
			String endDate = CommonUtils.checkNull(request.getParameter("ADD_DATE_lte"));
			ComplaintReport po=service.getComlaintReportData(startDate,endDate);
